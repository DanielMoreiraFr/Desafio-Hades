...
ui/screens.py
─────────────
Full-screen renders: every distinct "moment" the player sees gets its own
function here. Imports components (widgets) and console (output), but has
zero knowledge of game rules — it only receives data and displays it.
"""
...
from ui.console     import console, ask, wait, pause
from ui.components  import (
    hp_bar, status_panel, action_table,
    boon_as_text, boon_choice_table, leaderboard_table,
)
...
def show_main_menu() -> str:
    """Render the main menu and return '1', '2', or '3'."""
    ...
    console.print(Panel(
        table,
        title="[bold yellow]🔱  H A D E S  —  Underworld Roguelite  🔱[/bold yellow]",
        border_style="yellow",
        padding=(1, 4),
    ))
...
def show_room_heal(player: "Player", amount: int, is_boss: bool) -> None:  # type: ignore[name-defined]
    label = "[bold yellow]Boss Clear Bonus[/bold yellow]" if is_boss else "Room Clear"
    console.print(
        f"  🩹 {label}  [green]+{amount} HP[/green]  "
